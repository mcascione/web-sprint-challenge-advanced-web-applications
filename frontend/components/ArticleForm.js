...
export default function ArticleForm(props) {
  ...
  const {updateArticle, setCurrentArticleId, postArticle, currentArticle} = props;
  useEffect(() => {
    // ✨ implement
    // Every time the `currentArticle` prop changes, we should check it for truthiness:
    // if it's truthy, we should set its title, text and topic into the corresponding
    // values of the form. If it's not, we should reset the form back to initial values.
    currentArticle ? setValues({ title: currentArticle.title, text: currentArticle.text, topic: currentArticle.topic}) : setValues(initialFormValues);
  }, [currentArticle])
  ...
  const onSubmit = evt => {
    ...
    // reset the form and currentArticle
